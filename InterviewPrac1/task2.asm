...
addi $v0, $0, 5		#Call code to read integer
syscall
sw $v0, size		#Store read integer into size

#Create array the_list
lw $t0, size
sll $t0, $t0, 2		#Multiply size by 4 and store in $t0
addi $a0,$t0, 4		#Add 4 to $t0 for first element, length of the_list
...
slt $t0, $t0, $t1	#Check if i < len(the_list)	
beq $t0,$0,exit_loop	#Branch to exit_loop if i >= len(the_list)
...
add $t1,$t1,$t2		#Add i*4 to address of first element of the_list
lw $t1,4($t1)		#Load the_list[i]
...
end_of_loop:
lw $t0, i
addi $t0, $t0, 1		#Load i and increment i by 1
sw $t0, i
j while_i_lt_size
...
#Exit program
addi $v0, $0, 10		#Call code to exit program
syscall 
